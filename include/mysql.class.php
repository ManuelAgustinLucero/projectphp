...
class MySQL
{
	...
	*/
	...
	private function SetError($errorMessage = "", $errorNumber = 0) {
		...
		//KIKE// ACá atrapo los errores de mysql
		//echo $this->last_sql;echo "<br>";
		if($this->error_number > 0){
			echo $this->error_desc;echo "<br>";
			echo $this->error_number;echo "<br>";
			echo $this->last_sql;echo "<br>";
		}
		//////
	...
	public function CountRows($t,$w="1=1"){
		$r = $this->QueryArray("select count(*) as tmpTotal from $t where $w");
		return $r[0]['tmpTotal'];
	}	
	public function QSV($sql) {
		return $this->QuerySingleValue($sql);	
	}
	public function QSRA($sql, $resultType = MYSQL_ASSOC){
		return $this->QuerySingleRowArray($sql,$resultType);	
	}
	public function QA($sql, $resultType = MYSQL_ASSOC){
		return $this->QueryArray($sql, $resultType);
	}
	public function INSERTJ($tableName, $valuesArray){
		return $this->BuildSQLInsert($tableName, $valuesArray);
	}
	public function DELETEJ($tableName, $whereArray = null){
		return $this->BuildSQLDelete($tableName, $whereArray);
	}
	public function UPDATEJ($tableName, $valuesArray, $whereArray = null){
		return $this->BuildSQLUpdate($tableName, $valuesArray, $whereArray);
	}
}
?>