...
class MysqliDb
{
    ...
    public function QueryForCombo($sql){
        $a = $this->QueryArray($sql,MYSQL_NUM);
        return $a;        
    }
    ...
    public function QSRA($sql){
        return $this->rawQueryOne($sql);   
    }
    public function QA($sql){
        return $this->rawQuery($sql);
    }
    public function INSERTJ($tabla, $insert){
        return $this->insert($tabla, $insert);
    }
    public function UPDATEJ($tabla, $update, $w){
        $i = 0;
        $l = count($update);
        $u = '';
        while ($nombre_fruta = current($update)) {

            if($i > 0) {
                $u.= ', ' . key($update) . ' = "' . $update[key($update)] . '"';
            } else {
                $u.= key($update) . ' = "' . $update[key($update)] . '"';
            }
            next($update);
        ...
        $sql = 'UPDATE ' . $tabla . ' SET ' . $u . ' WHERE ' . $w;
        return $this->rawQuery($sql);
    }
    public function DELETEJ($tabla, $w = '1 = 1'){
        $sql = 'DELETE FROM ' . $tabla . ' WHERE ' . $w;
        return $this->rawQuery($sql);
    }
