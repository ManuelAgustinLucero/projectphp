<?
	...
	//Recorta el texto a X largo
	function recortar($string, $largo=NULL) {
	    if ($largo == NULL) {
	        $largo = 185;
	    }
	    
	    //Primero eliminamos las etiquetas html y luego cortamos el string
	    $recortado = substr(strip_tags($string), 0, $largo);
	    
	    //Si el texto es mayor que la longitud se agrega puntos suspensivos
	    if (strlen(strip_tags($string)) > $largo)
	        $recortado .= '...';
	    return $recortado;
	}

	//Limpia un stirng de tildes, ñ y espacios (similar a url_encode)	
	function amigable($string, $punto = false) {
	    $string = trim($string);
	    $string = str_replace(
	...
	function alertBS($msj, $tipo = 'danger', $dis = 0) {		
		...
		$res = '<div class="alert alert-' . $clase . $equis . '<strong>' . $titulo . '</strong> ' . $msj . '</div>';
		return $res;
	}
	...
	function LV($param,$html='s') {
		...
		if($html == 's') {
			...
			$param = str_ireplace("\"", "", $param);
			$param = str_ireplace("\\", "", $param);
			$param = str_ireplace("mysql","",$param);
			$param = str_ireplace("mssql","",$param);
			$param = str_ireplace("query","",$param);
			$param = str_ireplace("insert","",$param);
			$param = str_ireplace("select","",$param);
			$param = str_ireplace("update","",$param);
			$param = str_ireplace("delete","",$param);
			$param = str_ireplace("Character","",$param);
			$param = str_ireplace("MEMB_INFO","",$param);
			$param = str_ireplace("PHP_INFO","",$param);
			$param = str_ireplace("DROP","",$param);
			$param = str_ireplace("TRUNCATE","",$param);
			$param = str_ireplace("TABLE","",$param);*/
		}
